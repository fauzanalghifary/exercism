...
func TestKeep(t *testing.T) {
		...
		t.Run(
			tc.description, func(t *testing.T) {
				...
				case !reflect.DeepEqual(cp, tc.list):
					t.Fatalf(
						"Keep(%#v, fn) should not modify its argument. %#v should stay %#v",
						tc.list,
						cp,
						tc.list,
					)
	...
	t.Run(
		"keeps strings", func(t *testing.T) {
...
func TestDiscard(t *testing.T) {
	for _, tc := range discardIntTests {
		t.Run(
			tc.description, func(t *testing.T) {
				...
				case !reflect.DeepEqual(cp, tc.list):
					t.Fatalf(
						"Discard(%#v, fn) should not modify its argument. %#v should stay %#v",
						tc.list,
						cp,
						tc.list,
					)
	...
	t.Run(
		"discards strings", func(t *testing.T) {
			zword := func(x string) bool { return strings.HasPrefix(x, "z") }
			list := []string{"apple", "zebra", "banana", "zombies", "cherimoya", "zealot"}
	...
	t.Run(
		"discards lists", func(t *testing.T) {
			has5 := func(list []int) bool {
				for _, entry := range list {
					if entry == 5 {
						return true
